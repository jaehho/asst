...
- **Lists are ordered and archived in the window,** as Planify keeps them: `window.toml`, not the server's `calendar-order`, which shared and read-only lists couldn't take anyway. An archived list still syncs and its reminders still ring; it is only out of sight (sidebar, views, counts, the tray). The ring beside a list fills with its completed share, from counts asstd keeps per list.
...
- iOS parks dateless relative alarms at `19760401T005545Z`. asst ignores that alarm, and proximity alarms too.
...
- **Linked notes.** A task can link to notes in the folder Nextcloud Notes keeps, as the Nextcloud client puts it on this disk (`~/Nextcloud/Notes`; `notes` in `config.toml`). Most tasks need none. Copying every task's notes into Notes was the other way, and was turned down: two editable copies to reconcile, and a file per task.
  - A link is the note's path in the folder, one `X-ASST-NOTE` property each, which survives an iPhone edit but doesn't show there. asst makes notes (named after the task, starting with it as a `# ` heading, which Notes reads as the title) and never reads or edits one.
  - **Links follow moves.** asstd watches the folder and, once it has been quiet for a second, replays the renames in order: renamed twice, or its folder moved, a note keeps its links. A save that renames the note away and writes a new one leaves the link, since a file is at the old path again. A move while asstd isn't running goes unseen, and the window strikes that link through. The Nextcloud client's file IDs would survive that too, but they live in the client's own sync database.
  - **From either side.** A task's card links a note from the folder or makes one and opens it in the desktop's editor for Markdown; `asst attach`/`detach` do the same. From a note: `asst add --attach`, `asst ls --attached`, and in Neovim (`nvim/`) `:AsstTask` (quick add with the note attached), `:AsstAttach` (an open task to link) and `:AsstTasks` (the tasks linking it, opened in the window through `asst open`).
